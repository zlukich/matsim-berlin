...
/**
 * A default implementation of {@link RaptorIntermodalAccessEgress} returning a
 * new RIntermodalAccessEgress, which contains a list of legs (same as in the
 * input), the associated travel time as well as the disutility.
 *
...
public class BerlinRaptorIntermodalAccessEgress implements RaptorIntermodalAccessEgress {
	...
	private final MultiModeDrtConfigGroup multiModeDrtConfigGroup;
	// for randomization per person, per mode, per direction (but same random value
	// for one combination of this per routing request)
	...
	@Override
	public RIntermodalAccessEgress calcIntermodalAccessEgress(final List<? extends PlanElement> legs,
			RaptorParameters params, Person person, RaptorStopFinder.Direction direction) {
		...
		for (PlanElement pe : legs) {
			if (pe instanceof Leg) {
				...
				if (travelTime.isDefined()) {
					...
					utility += travelTime.seconds()
							* (scoringParams.getModes().get(mode).getMarginalUtilityOfTraveling()
									+ (-1) * scoringParams.getPerforming_utils_hr())
							/ 3600;
				}
				...
				if (distance != null && distance != 0.) {
					...
					utility += distance * scoringParams.getModes().get(mode).getMonetaryDistanceRate()
				...
				for (DrtConfigGroup drtConfigGroup : multiModeDrtConfigGroup.getModalElements()) {
					if (drtConfigGroup.getMode().equals(mode)) {
						DrtFareParams drtFareParams = drtConfigGroup.getDrtFareParams().orElseGet(null);
						double fare = 0.;
						if (distance != null && distance != 0.) {
				...
				for (IntermodalTripFareCompensatorConfigGroup compensatorCfg : interModalTripFareCompensatorsCfg
						.getIntermodalTripFareCompensatorConfigGroups()) {
					if (compensatorCfg.getDrtModes().contains(mode)
							&& compensatorCfg.getPtModes().contains(TransportMode.pt)) {
				...
				// check whether the same agente was already handled for the same direction (for
				// each trip it should always first handle all access stops and then all egress
				// stops)
				// assumes that the RaptorStopFinder handles by person, then by direction, then
				// by mode for each routing request (what DefaultRaptorStopFinder does)
				// -> same person, same direction should be all in one row without other agents
				// in between (otherwise will not work as expected)
				...
				IntermodalAccessEgressModeUtilityRandomization randomization = berlinCfg
						.getIntermodalAccessEgressModeUtilityRandomization(mode);
				if (randomization != null) {
					...
					if (utilityRandomizationSigma != 0.0) {
						...
						if (additiveRandomization == null) {
							 ...
							 * {@link org.matsim.core.router.costcalculators.RandomizingTimeDistanceTravelDisutilityFactory}
							 */
