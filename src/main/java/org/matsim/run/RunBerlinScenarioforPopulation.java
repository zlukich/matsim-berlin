...
public final class RunBerlinScenarioforPopulation {

	private static final Logger log = Logger.getLogger(RunBerlinScenarioforPopulation.class );
	...
	public static void main(String[] args) {
		...
		Id<Person> id = Id.createPersonId("386583101");
		Id<Person> id2 = Id.createPersonId("343957701");
		Plan plan1 = scenario.getPopulation().getPersons().get(id).getSelectedPlan();
		Plan plan2 = scenario.getPopulation().getPersons().get(id2).getSelectedPlan();
		PopulationFactory popf = PopulationUtils.getFactory();
		Population pop = PopulationUtils.createPopulation(ConfigUtils.createConfig());
		for(int i = 0; i <= 10;i++) {
			Id<Person> idi = Id.createPersonId("1"+i);
			Person person1 = popf.createPerson(idi);
			person1.addPlan(plan1);
			person1.setSelectedPlan(plan1);
			person1.getAttributes().putAttribute("subpopulation", "person");
			pop.addPerson(person1);
		}
		for(int i = 0; i <= 10;i++) {
			Id<Person> idi = Id.createPersonId("2"+i);
			Person person1 = popf.createPerson(idi);
			person1.addPlan(plan2);
			person1.setSelectedPlan(plan2);
			person1.getAttributes().putAttribute("subpopulation", "person");
			pop.addPerson(person1);
		}
		PopulationUtils.writePopulation(pop, "PopulationforMiBTest.xml");
